...
class CompraService(BaseService):
    ...
    def calcular_total(self, passageiro_id: int, cupom_id: int = None) -> dict:
        ...
        Args:
            ...
            cupom_id (int, optional): ID do cupom a ser aplicado.
        ...
        valor_passagem = float(ultima_passagem.get("valor_pago", ultima_passagem.get("valor_base_passagem", 0)))
    ...
    def criar_cupom_manual(self, passageiro_id: int, percentual_desconto: float, 
        ...
        try:
            ...
            if percentual_desconto < 0 or percentual_desconto > 1:
                raise ValueError("O percentual de desconto deve estar entre 0 e 1 (0% a 100%)")
            
            # Calcular data de validade
            data_validade = date.today() + timedelta(days=dias_validade)
            
            # Criar o cupom
            cupom_id = self.service.criar_cupom(passageiro_id, percentual_desconto, data_validade)
    ...
    def aplicar_cupom_na_compra(self, passageiro_id: int, cupom_id: int, 
        valor_compra: float) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, 
                detail=f"Erro ao aplicar cupom: {str(e)}"
            )
    ...
    def salvar_compra(self, compra_data: dict) -> int:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Erro ao salvar compra: {str(e)}"
            )